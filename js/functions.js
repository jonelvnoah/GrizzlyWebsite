  ...
  $(".btnContactUs").on("click",function () {
    if(btnContactUs){
      $(".btnClose").trigger("click");
        ...
        $(".contactContent").animate({height: "+=370"},300);
        $(this).addClass("on");
        btnContactUs = true;
    }
  });
...
// fixed NavBar
  $(window).scroll(function(){
    ...
    if(topScr > 55) {
        if (btnContactUs) {
            $(".wrapMenu").css({top: 372});
        } else {
            $(".wrapMenu").css({top: 0});
            $(".contactContent").css({"height": 0});
        }
        $(".wrapMenu").addClass("fixed");
        $(".contactContent").addClass("fixed");
    }else {
      if(btnContactUs){
        $(".wrapMenu").css({top: ""});
      }else{
        $(".contactContent").css({"height": 10});
      }
      $(".wrapMenu").removeClass("fixed");
        $(".contactContent").removeClass("fixed");
    }
  });

$(".chat.col-md-2").on("click",function () {
    $(this).toggleClass("on");
});
